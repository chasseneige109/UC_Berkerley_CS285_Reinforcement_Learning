...
class PGAgent(nn.Module):
    ...
    def _discounted_return(self, rewards: Sequence[float]) -> Sequence[float]:
        ...
        Note that all entries of the output list should be the exact same because each sum is from 0 to T (and doesn't involve t)!
        """
        T = len(rewards)
        gammas = np.array([self.gamma ** i for i in range(0, T)])
        total = np.sum(gammas * rewards)
        
        return np.full(T, total)
    ...
    def _discounted_reward_to_go(self, rewards: Sequence[float]) -> Sequence[float]:
        ...
        r_t_g = np.zeros(T)
        for t in range(T):
            gammas = np.array([self.gamma ** (t_ - t) for t_ in range(t, T)])
            r_t_g[t] = np.sum(gammas * rewards[t:])
        return r_t_g